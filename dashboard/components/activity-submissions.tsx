    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-2 border-b border-border pb-4">
        <div className="relative min-w-[220px] flex-1">
          ...
          <Input aria-label="Search Submissions" className="pl-9" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search Discord or FP ID" />
        </div>
        <select aria-label="Outcome Filter" className="h-9 rounded-md border border-input bg-background px-3 text-sm" value={outcome} onChange={(event) => setOutcome(event.target.value as ActivitySubmissionOutcome | "all")}>
      ...
      {loading ? (
        ...
        <div className="rounded-md border border-dashed border-border py-12 text-center text-sm text-muted-foreground">No submissions</div>
      ) : (
            ...
            <tbody>
              {filtered.map((submission) => (
                <tr key={submission.id} className="border-b border-border last:border-0">
                  ...
                  <td className="px-3 py-2.5"><span className={submission.outcome === "winner" ? "fp-pill fp-pill-success" : "fp-pill fp-pill-muted"}>{submission.outcome}</span></td>
