"use client"
import { Suspense } from "react"
import { signIn } from "next-auth/react"
import { useSearchParams } from "next/navigation"

export const dynamic = "force-dynamic"
import { Button } from "@/components/ui/button"

function BrandMarkLg() {
  return (
    <div
      className="fp-brand-mark mx-auto mb-5"
      style={{ width: 44, height: 44 }}
      aria-hidden="true"
    >
      <svg width="22" height="22" viewBox="0 0 16 16" fill="none">
        <path d="M8 1.5 L14.5 8 L8 14.5 L1.5 8 Z" stroke="rgba(255,255,255,0.9)" strokeWidth="1.2" strokeLinejoin="round"/>
        <path d="M8 5 L11 8 L8 11 L5 8 Z" fill="rgba(255,255,255,0.9)"/>
      </svg>
    </div>
  )
}

function LoginForm() {
  const params = useSearchParams()
  const error = params?.get("error")

  return (
    <div className="relative w-full max-w-[380px]">
      {/* Outer violet halo */}
      <div
        aria-hidden="true"
        className="pointer-events-none absolute -inset-8 -z-10 opacity-60"
        style={{
          background:
            "radial-gradient(60% 50% at 50% 0%, hsl(var(--primary) / 0.20), transparent 70%)",
        }}
      />

      <div className="relative rounded-xl border border-border bg-card/95 backdrop-blur-md p-8 shadow-[0_30px_80px_-30px_hsl(var(--primary)/0.35),inset_0_1px_0_rgba(255,255,255,0.03)]">
        <BrandMarkLg />

        <div className="space-y-1.5 text-center mb-7">
          <h1 className="text-[22px] font-semibold tracking-tight text-foreground">Fortune Purple</h1>
          <p className="text-[13px] text-muted-foreground">Discord Bot 管理后台</p>
        </div>

        {error === "AccessDenied" && (
          <div className="mb-5 rounded-md border border-destructive/30 bg-destructive/10 p-3 text-center text-[12.5px] text-destructive">
            无权访问:你的账号没有管理员身份组
          </div>
        )}

        <Button
          className="w-full h-10 text-[13.5px]"
          onClick={() => signIn("discord", { callbackUrl: "/dashboard/embeds" })}
        >
          <svg aria-hidden="true" className="h-4 w-4" viewBox="0 0 24 24" fill="currentColor">
            <path d="M20.317 4.37a19.791 19.791 0 0 0-4.885-1.515.074.074 0 0 0-.079.037c-.21.375-.444.864-.608 1.25a18.27 18.27 0 0 0-5.487 0 12.64 12.64 0 0 0-.617-1.25.077.077 0 0 0-.079-.037A19.736 19.736 0 0 0 3.677 4.37a.07.07 0 0 0-.032.027C.533 9.046-.32 13.58.099 18.057c.002.022.015.043.033.055a19.88 19.88 0 0 0 5.993 3.03.078.078 0 0 0 .084-.028 14.09 14.09 0 0 0 1.226-1.994.076.076 0 0 0-.041-.106 13.107 13.107 0 0 1-1.872-.892.077.077 0 0 1-.008-.128 10.2 10.2 0 0 0 .372-.292.074.074 0 0 1 .077-.01c3.928 1.793 8.18 1.793 12.062 0a.074.074 0 0 1 .078.01c.12.098.246.198.373.292a.077.077 0 0 1-.006.127 12.299 12.299 0 0 1-1.873.892.077.077 0 0 0-.041.107c.36.698.772 1.362 1.225 1.993a.076.076 0 0 0 .084.028 19.839 19.839 0 0 0 6.002-3.03.077.077 0 0 0 .032-.054c.5-5.177-.838-9.674-3.549-13.66a.061.061 0 0 0-.031-.03z" />
          </svg>
          用 Discord 登录
        </Button>

        <div className="mt-5 flex items-center justify-center gap-2 text-[11.5px] text-muted-foreground">
          <span>仅限授权管理员</span>
          <span className="h-1 w-1 rounded-full bg-muted-foreground/50" aria-hidden="true" />
          <span>OAuth 2.0 安全登录</span>
        </div>
      </div>

      <div className="mt-5 flex items-center justify-center gap-2 text-[11px] text-muted-foreground/70">
        <span>Fortune Purple Dashboard</span>
        <span>·</span>
        <span>v2.4</span>
      </div>
    </div>
  )
}

export default function LoginPage() {
  return (
    <div className="flex min-h-screen items-center justify-center bg-background p-4">
      <Suspense fallback={<div className="text-sm text-muted-foreground">Loading…</div>}>
        <LoginForm />
      </Suspense>
    </div>
  )
}
