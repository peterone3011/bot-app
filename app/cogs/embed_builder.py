...
_STORES_BY_BOT: dict[int, EmbedMessageStore] = {}
_STORES_BY_MESSAGE_ID: dict[str, EmbedMessageStore] = {}


def _store_for_bot(bot: commands.Bot) -> EmbedMessageStore:
    try:
        return _STORES_BY_BOT[id(bot)]
    except KeyError as exc:
        raise RuntimeError("Embed builder storage is unavailable") from exc


def _register_messages(store: EmbedMessageStore, messages: list[dict[str, Any]]) -> list[dict[str, Any]]:
    for message in messages:
        _STORES_BY_MESSAGE_ID[message["id"]] = store
    return messages


async def _messages_for_bot(bot: commands.Bot) -> list[dict[str, Any]]:
    store = _store_for_bot(bot)
    return _register_messages(store, await store.list())


async def aget_message(message_id: str) -> dict[str, Any] | None:
    store = _STORES_BY_MESSAGE_ID.get(message_id)
    if store is None:
        return None
    return await store.get(message_id)


async def aupsert_message(message: dict[str, Any]) -> None:
    message_id = message.get("id")
    store = _STORES_BY_MESSAGE_ID.get(message_id)
    if store is None:
        raise RuntimeError("Embed record is not associated with a project")
    await store.upsert(message)


async def adelete_message(message_id: str) -> None:
    store = _STORES_BY_MESSAGE_ID.get(message_id)
    if store is None:
        raise RuntimeError("Embed record is not associated with a project")
    await store.delete(message_id)
    _STORES_BY_MESSAGE_ID.pop(message_id, None)


async def last_used_color(bot: commands.Bot) -> int | None:
    colored = [m for m in await _messages_for_bot(bot) if m.get("color") is not None]
    if not colored:
        return None
...
async def _sorted_messages(bot: commands.Bot) -> list[dict[str, Any]]:
    order = {"scheduled": 0, "draft": 1, "published": 2}
    return sorted(
        await _messages_for_bot(bot),
        key=lambda m: (
...
class BuilderMainView(discord.ui.View):
    ...
    async def back_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        messages = await _sorted_messages(interaction.client)
        if messages:
            await interaction.response.edit_message(
...
class NewMessageChannelSelect(discord.ui.ChannelSelect):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        store = _store_for_bot(interaction.client)
        msg = new_draft(channel.id, color=await last_used_color(interaction.client))
        _register_messages(store, [msg])
        await aupsert_message(msg)
        await interaction.response.edit_message(
...
class MessageSelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        msg = await aget_message(self.values[0])
        if msg is None:
...
class EmbedBuilderCog(commands.Cog):
    def __init__(self, runtime: ProjectRuntime):
        if runtime.embed_store is None:
            raise RuntimeError("Embed builder requires initialized project storage")
        self.runtime = runtime
        self.bot = runtime.bot
        _STORES_BY_BOT[id(self.bot)] = runtime.embed_store
        self.send_loop.start()

    def cog_unload(self):
        self.send_loop.cancel()
        _STORES_BY_BOT.pop(id(self.bot), None)

    @tasks.loop(seconds=60)
    async def send_loop(self):
        now = datetime.now(CST)
        for msg in list(await _messages_for_bot(self.bot)):
    ...
    async def embed_cmd(self, interaction: discord.Interaction):
        ...
        messages = await _sorted_messages(self.bot)
        if messages:
    ...
    async def edit_embed_cmd(self, interaction: discord.Interaction, message_link: str):
        ...
        _register_messages(_store_for_bot(self.bot), [draft])
        await aupsert_message(draft)
        await interaction.followup.send(
...
async def install(runtime: ProjectRuntime) -> None:
    cog = EmbedBuilderCog(runtime)
    await runtime.bot.add_cog(cog)
    ...
    async def edit_embed_menu(interaction: discord.Interaction, message: discord.Message):
        if message.author != runtime.bot.user:
            await interaction.response.send_message(
        ...
        if not message.embeds:
            ...
            return
        draft = draft_from_message(message)
        _register_messages(_store_for_bot(runtime.bot), [draft])
        await aupsert_message(draft)
        await interaction.response.send_message(
            embed=build_builder_embed(draft),
            view=BuilderMainView(draft["id"]),
    ...
    runtime.bot.tree.add_command(edit_embed_menu)
